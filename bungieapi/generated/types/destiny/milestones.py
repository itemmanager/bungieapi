...
class DestinyMilestone:
    ...
    start_date: str  # If known, this is the date when the event last began or refreshed. It will only be populated for events with fixed and repeating start and end dates.
    end_date: str  # If known, this is the date when the event will next end or repeat. It will only be populated for events with fixed and repeating start and end dates.
...
class DestinyMilestoneActivity:
    ...
    activity_mode_type: int  # The enumeration equivalent of the most specific Activity Mode under which this activity is played.
...
class DestinyMilestoneActivityCompletionStatus:
    ...
    completed: bool  # If the activity has been "completed", that information will be returned here.
    phases: t.Sequence[
...
class DestinyMilestoneActivityPhase:
    ...
    complete: bool  # Indicates if the phase has been completed.
    phase_hash: int  # In DestinyActivityDefinition, if the activity has phases, there will be a set of phases defined in the "insertionPoints" property. This is the hash that maps to that phase.
...
class DestinyMilestoneRewardCategory:
    ...
    reward_category_hash: int  # Look up the relevant DestinyMilestoneDefinition, and then use rewardCategoryHash to look up the category info in DestinyMilestoneDefinition.rewards.
    entries: t.Sequence[
...
class DestinyMilestoneRewardEntry:
    ...
    earned: bool  # If TRUE, the player has earned this reward.
    redeemed: bool  # If TRUE, the player has redeemed/picked up/obtained this reward. Feel free to alias this to "gotTheShinyBauble" in your own codebase.
...
class DestinyMilestoneContent:
    ...
    about: str  # The "About this Milestone" text from the Firehose.
    status: str  # The Current Status of the Milestone, as driven by the Firehose.
    tips: t.Sequence[str]  # A list of tips, provided by the Firehose.
    item_categories: t.Sequence[
...
class DestinyPublicMilestone:
    ...
    start_date: str  # If known, this is the date when the Milestone started/became active.
    end_date: str  # If known, this is the date when the Milestone will expire/recycle/end.
    order: int  # Used for ordering milestones in a display to match how we order them in BNet. May pull from static data, or possibly in the future from dynamic information.
...
class DestinyPublicMilestoneChallenge:
    ...
    activity_hash: int  # IF the Objective is related to a specific Activity, this will be that activity's hash. Use it to look up the DestinyActivityDefinition for additional data to show.
