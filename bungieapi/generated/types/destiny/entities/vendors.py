...
class DestinyVendorComponent:
    ...
    can_purchase: bool  # If True, you can purchase from the Vendor.
    ...
    seasonal_rank: int  # If this vendor has a seasonal rank, this will be the calculated value of that rank. How nice is that? I mean, that's pretty sweeet. It's a whole 32 bit integer.
...
class DestinyVendorCategory:
    ...
    display_category_index: int  # An index into the DestinyVendorDefinition.displayCategories property, so you can grab the display data for this category.
    item_indexes: t.Sequence[
        int
...
class DestinyVendorSaleItemComponent:
    ...
    sale_status: "VendorItemStatus"  # A flag indicating whether the requesting character can buy the item, and if not the reasons why the character can't buy it.
    required_unlocks: t.Sequence[
        int
