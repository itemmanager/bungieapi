...
class DestinyProgressionRewardItemState(Flag):
...
class EquippingItemBlockAttributes(Flag):
    NONE = 0
    EQUIP_ON_ACQUIRE = 1
...
class DestinyCollectibleState(Flag):
...
class DestinyPartyMemberStates(Flag):
...
class DestinyJoinClosedReasons(Flag):
...
class DestinyVendorItemState(Flag):
