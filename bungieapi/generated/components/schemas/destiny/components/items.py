...
class DestinyItemReusablePlugsComponent:
    plugs: t.Optional[
        t.Mapping[str, t.Sequence["DestinyItemPlugBase"]]
...
class DestinyItemPlugObjectivesComponent:
    objectives_per_plug: t.Optional[
    ...
    def to_json(self) -> t.Mapping[str, t.Any]:
        return {
            "objectivesPerPlug": to_json(self.objectives_per_plug),
        }
...
class DestinyItemPlugComponent:
    ...
    can_insert: t.Optional[
        bool
    ] = None  # If true, this plug has met all of its insertion requirements. Big if true.
    enable_fail_indexes: t.Optional[
        t.Sequence[int]
    ] = None  # If a plug is not enabled, this will be populated with indexes into the plug item definition's plug.enabledRules property, so that you can show the reasons why it is not enabled. This list will be empty if the plug is enabled.
    enabled: t.Optional[
        bool
    ] = None  # If true, this plug will provide its benefits while inserted.
    insert_fail_indexes: t.Optional[
        t.Sequence[int]
    ] = None  # If the plug cannot be inserted for some reason, this will have the indexes into the plug item definition's plug.insertionRules property, so you can show the reasons why it can't be inserted. This list will be empty if the plug can be inserted.
    plug_item_hash: t.Optional[
        int
    ] = None  # The hash identifier of the DestinyInventoryItemDefinition that represents this plug.
    ...
    def to_json(self) -> t.Mapping[str, t.Any]:
        return {
            ...
            "canInsert": to_json(self.can_insert),
            "enabled": to_json(self.enabled),
            "insertFailIndexes": to_json(self.insert_fail_indexes),
